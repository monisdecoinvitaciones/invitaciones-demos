...
const InvitacionPage = () => {
  ...
  if (!isAuthenticated && errorStatus !== "EXPIRED") {
    return (
      <div className="invitacion-status-wrapper" id="rsvp">
        <div className="status-card auth-card" style={{ textAlign: 'center' }}>
          <h2 className="txt-large font-body" style={{ color: '#000', margin: '15px 0' }}>
            CONFIRMAR ASISTENCIA
          </h2>

          <p className="txt-medium" style={{ color: '#333' }}>Ingresa tu código de invitado para confirmar tu asistencia.</p>
          ...
          <button 
            ...
            className="btn-rsvp"
            disabled={buscando}
            style={{ width: '100%', maxWidth: '250px', backgroundColor: '#d38c9d' }}
          >
            {buscando ? "BUSCANDO..." : "INGRESAR"}
          </button>

          <div style={{ marginTop: '25px', borderTop: '1px solid #F8D8DE', paddingTop: '20px' }}>
            <p style={{ fontSize: '0.85rem', color: '#666', marginBottom: '10px' }}>¿Eres cliente y quieres ver el panel?</p>
            <button 
        ...
        {isModalOpen && (
          <div className="modal-overlay" onClick={() => setIsModalOpen(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <button className="modal-close" onClick={() => setIsModalOpen(false)} style={{ color: '#000' }}><X size={24} /></button>
              
              <div className="modal-header">
                <div className="modal-icon-bg" style={{ backgroundColor: '#F8D8DE', color: '#000' }}>
                  <LayoutDashboard size={32} />
                </div>
                <h3 className="font-body" style={{ margin: '15px 0 10px', fontSize: '1.5rem', color: '#000' }}>¡Olvídate de papel y Excel!</h3>
              ...
              <div className="credentials-box" style={{ 
                ...
                <p style={{ fontSize: '0.7rem', color: '#000', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '10px' }}>
                  Datos de acceso:
                </p>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                  <span style={{ color: '#666', fontSize: '0.9rem' }}>Usuario:</span>
                  <span style={{ fontWeight: '600', color: '#000', fontSize: '0.9rem' }}>demo@invimon.com</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#666', fontSize: '0.9rem' }}>Contraseña:</span>
                  <span style={{ fontWeight: '600', color: '#000', fontSize: '0.9rem' }}>invimon2026</span>
                </div>
              </div>
              ...
              <div className="modal-footer">
                ...
                >
                  ...
                  <span style={{ flex: 1, textAlign: 'center' }}>Ir al Panel</span>
    ...
    return (
      <div className="invitacion-status-wrapper">
        <div className="status-card expired">
          ...
          <h2 className="txt-large font-body" style={{ color: '#000' }}>Fecha Límite Excedida</h2>
          <p style={{ color: '#333' }}>Lo sentimos, el periodo de confirmación terminó el día:</p>
          <strong style={{ color: '#B8445E' }}>{fechaLimiteExposicion}</strong>
        </div>
      </div>
    );
  }
      ...
      <div className="invitacion-status-wrapper" id="rsvp">
        <div className="status-card success">
          <div className="header-success">
            <h2 className="txt-large font-body" style={{ color: '#000' }}>
              HOLA {invitadoData.nombre.toUpperCase()}
            </h2>
            {declinoAsistencia ? (
              <p style={{ color: '#333' }}>Has registrado que no podrás asistir. ¡Te extrañaremos!</p>
            ) : (
              <>
                <p style={{ color: '#333' }}>¡Asistencia registrada con éxito!</p>
                <p style={{ color: '#B8445E', fontWeight: 'bold' }}>Ya puedes descargar tus pases oficiales.</p>
              </>
            )}
          </div>
          ...
          <div className="info-resumen">
            {declinoAsistencia ? (
              ...
              <>
                <div className="dato" style={{ borderBottom: '1px solid #F8D8DE', paddingBottom: '10px' }}>
                  <span style={{ color: '#666' }}>Confirmados:</span>
                  <strong style={{ color: '#000' }}>{invitadoData.confirmadosLista}</strong>
                </div>
                ...
                <button 
                  ...
                  style={{ margin: '20px auto', display: 'block', backgroundColor: '#000' }}
                >
                  {isGenerating ? "GENERANDO..." : "DESCARGAR PASES"}
                </button>

                <div className="qr-container" style={{ display: 'flex', justifyContent: 'center', padding: '10px', background: '#fff', borderRadius: '15px' }}>
                  <QRCodeSVG 
                ...
                <div className="dato" style={{ textAlign: 'center', justifyContent: 'center' }}>
                  <span style={{ color: '#333', fontSize: '0.9rem' }}>Presenta este código en la entrada del evento.</span>
                </div>
    ...
    <div id="rsvp"> 
      ...
      {fechaLimiteExposicion && (
        <p style={{
          ...
          Fecha límite para confirmar: {fechaLimiteExposicion}
        </p>
      )}
    </div>
  );
